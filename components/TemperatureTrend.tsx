...
const TemperatureTrend: FC<TemperatureTrendProps> = ({ hourly, unit }) => {
  const { predicted, trendDirection, sparklinePoints } = useMemo(() => {
    const temps = hourly.slice(0, HISTORY_WINDOW).map((h) => h.temp_c ?? 0);
    if (temps.length < 3) return { predicted: [], trendDirection: 'flat' as const, sparklinePoints: '' };
    ...
    return { predicted, trendDirection, sparklinePoints };
  }, [hourly]);
